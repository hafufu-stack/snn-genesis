"""
Phase 91: Differential PCA on Mistral-7B — Finding Mistral's Reasoning Direction
==================================================================================

Phase 87 discovered Qwen's "true reasoning axis" via Differential PCA and
achieved 50% solve rate (absolute record). This experiment applies the same
methodology to Mistral-7B to answer:

  1. Does Mistral ALSO have a hidden discriminant axis?
  2. Does noise in Mistral's discriminant direction help more than PC 257+?
  3. Does Mistral's discriminant axis overlap with its "safe subspace" (PC 257+)?
...
Method:
  ...
  3. Differential PCA → discriminant directions
  4. Test noise injection along discriminant directions
...
def compute_differential_pca(labeled_games):
    ...
    n_solved_games = len(solved_states)
    n_failed_games = len(failed_states)
    print(f"    Solved games: {n_solved_games}, Failed games: {n_failed_games}")

    # Edge case: need at least 1 game in each class to compute discriminant
    if n_solved_games == 0 or n_failed_games == 0:
        print(f"    WARNING: Only one class present (solved={n_solved_games}, failed={n_failed_games})")
        print(f"    Falling back to ALL game means for PCA with random discriminant direction")
        all_means = solved_states if n_solved_games > 0 else failed_states
        if len(all_means) == 0:
            # Extreme edge: no games at all — use dummy
            raise ValueError("No games collected at all — cannot compute PCA")
        all_means = np.array(all_means)
        global_mean = all_means.mean(axis=0)
        centered = all_means - global_mean
        U, S, Vt = np.linalg.svd(centered, full_matrices=False)
        # Use PC1 as the discriminant direction (best we can do)
        diff_unit = Vt[0].copy()
        diff_norm = 0.0
        info = {
            "n_solved": n_solved_games,
            "n_failed": n_failed_games,
            "diff_vector_norm": 0.0,
            "fallback": True,
            "fallback_reason": "single_class_only",
            "cosine_with_standard_pca": None,
            "subspace_overlap": {"top64": None, "mid_band": None, "pc257_plus": None},
            "top_separating_pcs": [],
            "separation_scores_top50_mean": 0.0,
        }
    ...
    cos_with_pca = None
    top64_overlap = 0.0; mid_overlap = 0.0; pc257_overlap = 0.0
    ...
    if os.path.exists(pca_path):
        ...
        # Check overlap with PC 257+ subspace
        pc257_overlap = float(np.sum(cos_sims[256:]**2))
        top64_overlap = float(np.sum(cos_sims[:64]**2))
        mid_overlap = float(np.sum(cos_sims[64:256]**2))
        print(f"    Discriminant axis overlap with standard PCA subspaces:")
        print(f"      Top-64 PCs: {top64_overlap*100:.1f}%")
        print(f"      Mid-band (65-256): {mid_overlap*100:.1f}%")
        print(f"      PC 257+: {pc257_overlap*100:.1f}%")

    # PCA on centered data
    all_game_means = np.vstack([solved_states, failed_states])
    labels = np.array([1]*n_solved_games + [0]*n_failed_games)
    ...
    info = {
        ...
        "subspace_overlap": {
            "top64": round(float(top64_overlap), 4) if cos_with_pca else None,
            "mid_band": round(float(mid_overlap), 4) if cos_with_pca else None,
            "pc257_plus": round(float(pc257_overlap), 4) if cos_with_pca else None,
        },
        "top_separating_pcs": [
...
def visualize(all_results, diff_info):
    ...
    fig.suptitle("Phase 91: Differential PCA — Mistral's Reasoning Direction",
    ...
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "phase91_mistral_diff_pca.png")
...
def run_phase91(model, tok, device):
    print(f"\n{'='*80}")
    print(f"  Phase 91: Differential PCA — Mistral Reasoning Direction")
    ...
    # Save differential PCA for reuse (Phase 94, 95)
    pca_save = os.path.join(RESULTS_DIR, "phase91_diff_pca.npz")
    np.savez_compressed(pca_save, Vt=diff_Vt, S=diff_S, diff_unit=diff_unit)
    print(f"  Differential PCA saved: {pca_save}")

    all_results = {
        "experiment": "Phase 91: Differential PCA — Mistral Reasoning Direction",
        "model": MODEL_SHORT,
    ...
    for cond_idx, cond in enumerate(CONDITIONS):
        ...
        # Intermediate save
        results_path = os.path.join(RESULTS_DIR, "phase91_log.json")
...
def main():
    ...
    results, elapsed = run_phase91(model, tok, device)

    print(f"\n{'='*80}")
    print(f"  Phase 91 COMPLETE: {elapsed/60:.1f} min ({elapsed/3600:.1f} hours)")
    print(f"{'='*80}")
