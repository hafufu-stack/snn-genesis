"""
Phase 83: Mid-Band PCA Noise — Fine Structure of the Reasoning Manifold
========================================================================

Phase 71 showed: PCA-top4 noise KILLS reasoning (6.7% = baseline).
Phase 79 showed: Orthogonal complement ≈ random (46.7% both).

This experiment probes the FINE STRUCTURE between "top" and "bottom":
  Which PCA band is most effective for stochastic resonance?

PCA Bands:
  1. baseline:     No noise
  2. random:       Full random (reference, ~46%)
  3. pca_top4:     PC 1-4 only (negative control, ~6.7%)
  4. pca_mid_5_16: PC 5-16 only
  5. pca_mid_17_64: PC 17-64 only
  6. pca_mid_65_256: PC 65-256 only
  7. pca_bottom:   PC 257-4096 only

All use Flash Annealing (first-10 linear σ decay), N=30 per condition.
Step 1: Collect hidden states (50 baseline games).
Step 2: PCA decomposition.
Step 3: Run conditions with band-limited noise.

Total: 7 conditions × N=30 = 210 games + 50 collection = ~6 hours
"""
...
class BandNoiseHook:
    ...
    For band PC[start:end]:
      1. Generate random coefficients in the band subspace
      2. Project coefficients onto the corresponding PC vectors
      3. Apply Flash Annealing schedule
    """
    def __init__(self):
        self.active = False
        self.sigma = BASE_SIGMA
        self.mode = "random"  # "random", "pca_band"
        self.band_basis = None  # (band_size, d) — PC vectors for this band
        self.handle = None
    ...
    def setup_band(self, Vt_band, device='cuda'):
        """Set noise to a specific PCA band.
        
        Vt_band: (band_size, d) — PC vectors for this band
        """
        self.mode = "pca_band"
    ...
    def register(self, model, layer_idx=LAYER_IDX):
        ...
        def hook_fn(module, args):
            ...
            if hook_obj.mode == "random":
                ...
                V = hook_obj.band_basis  # (band_size, d)
                band_k = V.shape[0]
                b, s, d = hs.shape
                coeffs = torch.randn(b, s, band_k, dtype=hs.dtype, device=hs.device) * hook_obj.sigma
                noise = coeffs @ V  # (b, s, d)
                # Rescale: band noise has fewer dims, so scale up to match
                # energy of full-rank noise (√(d/band_k) factor)
...
CONDITIONS = [
    ...
    {"name": "random",           "mode": "random",     "pc_start": 0,   "pc_end": 4096},
    ...
    {"name": "pca_bottom",       "mode": "pca_band",   "pc_start": 256, "pc_end": 4096},
]


def run_phase83(model, tok, device):
    print(f"\n{'='*80}")
    print(f"  Phase 83: Mid-Band PCA Noise")
    ...
    bands = [(0, 4), (4, 16), (16, 64), (64, 256), (256, min(len(S), 4096))]
    ...
    all_results = {
        "experiment": "Phase 83: Mid-Band PCA Noise",
        "model": MODEL_SHORT,
        "layer": LAYER_IDX,
        "sigma": BASE_SIGMA,
    ...
    print(f"\n  === Band Comparison vs Random ===")
    for cond in all_results["conditions"]:
        if cond["mode"] == "pca_band":
            table = [[cond["n_solved"], cond["n_total"]-cond["n_solved"]],
    ...
    results_path = os.path.join(RESULTS_DIR, "phase83_log.json")
...
def main():
    ...
    results, elapsed = run_phase83(model, tok, device)

    print(f"\n{'='*80}")
    print(f"  Phase 83 COMPLETE: {elapsed/60:.1f} min ({elapsed/3600:.1f} hours)")
    print(f"{'='*80}")
