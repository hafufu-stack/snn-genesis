"""
Phase 92: Aha! Steering on Qwen — Deterministic Success Vector
===============================================================

Phase 87 found Qwen's discriminant axis (Diff-PCA success direction) and
achieved 50% with stochastic noise in that direction. Phase 89 showed
direction=31%, randomness=69% in PC 257+ for Mistral.

Key question: In the TRUE reasoning direction (Diff-PCA), does the
ratio change? Is deterministic steering MORE effective when applied
in the discriminant direction vs arbitrary PCA directions?

Method:
  1. Load Phase 87's Diff-PCA vectors (diff_unit = solved_mean - failed_mean)
  2. Test deterministic steering (Phase 86 pattern) in the discriminant direction
  3. Compare: stochastic noise vs deterministic offset vs combination

Conditions (N=30 each):
  1. baseline:           No noise
  2. diff_pca_noise:     Stochastic noise in top-10 Diff-PCs (Phase 87 replication)
...
def load_model():
    ...
    tok = AutoTokenizer.from_pretrained(MODEL_NAME, use_fast=True, trust_remote_code=True, local_files_only=True)
...
class AhaSteeringHook:
    ...
    in the Differential PCA discriminant direction.

    Modes:
      - "baseline":      No intervention
      - "diff_noise":    Stochastic noise in top-K Diff-PCs (Phase 87 replication)
      - "aha_steering":  Deterministic Δ offset in top-K Diff-PCs
      - "aha_plus_noise": Combined: half energy deterministic + half stochastic
      - "anti_aha":      Deterministic offset in -Δ direction
    """
    def __init__(self):
        self.active = False
        ...
        self.fixed_offset = None   # (d,) deterministic success direction
        self.handle = None
    ...
    def setup_diff_noise(self, Vt_band, device='cuda'):
        """Stochastic noise in Diff-PCA subspace."""
    ...
    def setup_aha_steering(self, Vt_band, diff_unit, device='cuda', anti=False):
        """Deterministic offset along the success direction.

        The success direction is projected onto the top-K Diff-PCs.
        """
        self.mode = "anti_aha" if anti else "aha_steering"
        V = Vt_band.to(device)

        # Project diff_unit onto the Diff-PCA subspace
        du = torch.tensor(diff_unit, dtype=torch.float32, device=device)
        # coeffs = V @ du → projection coefficients
        proj_coeffs = V.float() @ du  # (k,)
        proj = proj_coeffs @ V.float()  # (d,) projected vector
        proj_norm = proj.norm()
        if proj_norm > 1e-8:
            proj = proj / proj_norm  # unit vector in subspace

        if anti:
            proj = -proj  # Opposite direction

        self.fixed_offset = proj.half()
        self.band_basis = V

    def setup_aha_plus_noise(self, Vt_band, diff_unit, device='cuda'):
        """Combined: deterministic offset + stochastic noise."""
...
def visualize(all_results):
    ...
    fig.suptitle("Phase 92: Aha! Steering — Deterministic Success Vector on Qwen\n"
                 "Does the reasoning direction work better as deterministic offset or noise?",
    ...
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "phase92_aha_steering.png")
...
def run_phase92(model, tok, device):
    print(f"\n{'='*80}")
    print(f"  Phase 92: Aha! Steering — Deterministic Success Vector on Qwen")
    print(f"  {len(CONDITIONS)} conditions x N={N_PER_CONDITION}")
    print(f"{'='*80}")

    t0 = time.time()

    # Load Phase 87's Differential PCA
    ...
    if not os.path.exists(diff_pca_path):
        raise FileNotFoundError(f"Phase 87 Diff-PCA not found: {diff_pca_path}\n"
                                f"Run Phase 87 first to generate discriminant vectors.")

    print(f"  Loading Phase 87 Diff-PCA: {diff_pca_path}")
    data = np.load(diff_pca_path)
    diff_Vt = data["Vt"]        # (k, d) discriminant PCs
    diff_unit = data["diff_unit"]  # (d,) success direction unit vector
    print(f"  Loaded: Vt shape {diff_Vt.shape}, diff_unit norm {np.linalg.norm(diff_unit):.4f}")

    # Use top-10 discriminant PCs
    TOP_K = 10
    ...
    all_results = {
        "experiment": "Phase 92: Aha! Steering — Deterministic Success Vector on Qwen",
        "model": MODEL_SHORT,
    ...
    for cond_idx, cond in enumerate(CONDITIONS):
        ...
        # Intermediate save
        results_path = os.path.join(RESULTS_DIR, "phase92_log.json")
    ...
    print(f"\n  === Causal Interpretation ===")
    aha = next((c for c in all_results["conditions"] if c["condition"] == "aha_steering_top10"), None)
    anti = next((c for c in all_results["conditions"] if c["condition"] == "anti_aha"), None)
    if aha and anti:
        print(f"    Aha! steering:  {aha['solve_rate']*100:.1f}%")
        print(f"    Anti-aha:       {anti['solve_rate']*100:.1f}%")
        if aha["solve_rate"] > bl["solve_rate"] + 0.05:
            print(f"    → SUCCESS direction is causally beneficial")
        if anti["solve_rate"] < bl["solve_rate"] - 0.05:
            print(f"    → OPPOSITE direction is causally harmful")
...
def main():
    ...
    results, elapsed = run_phase92(model, tok, device)

    print(f"\n{'='*80}")
    print(f"  Phase 92 COMPLETE: {elapsed/60:.1f} min ({elapsed/3600:.1f} hours)")
    print(f"{'='*80}")
