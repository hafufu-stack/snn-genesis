"""
Phase 103: Prior Override N=100 Validation
==========================================

Phase 102 showed a +23.3pp differential (p=0.007) with N=30.
This replication uses N=100 per condition for publication-grade
statistical robustness.

Conditions (N=100 each, Mistral-7B):
  Standard Hanoi:
...
def visualize(all_results):
    ...
    fig.suptitle("Phase 103: Prior Override N=100 — Publication-Grade Validation\n"
                 f"Mistral-7B, N={N_PER_CONDITION} per condition",
    ...
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "phase103_prior_override_n100.png")
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"  Phase 103: Prior Override N=100 Validation")
    ...
    all_results = {
        "experiment": "Phase 103: Prior Override N=100 Validation",
        "model": MODEL_SHORT,
        ...
        "hypothesis": "Noise injection helps specifically when prior override is needed (N=100 validation)",
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "conditions": []
    }

    results_path = os.path.join(RESULTS_DIR, "phase103_log.json")
    ...
    # Fisher exact tests
    # Modified: aha vs baseline
    mod_aha = next(c for c in mod_conds if "aha" in c["condition"])
    table_mod = [[mod_aha["n_solved"], mod_aha["n_total"]-mod_aha["n_solved"]],
                 [mod_bl["n_solved"], mod_bl["n_total"]-mod_bl["n_solved"]]]
    _, p_mod = fisher_exact(table_mod)

    # Cross-rule: mod_aha vs std_aha
    std_aha = next(c for c in std_conds if "aha" in c["condition"])
    table_cross = [[mod_aha["n_solved"], mod_aha["n_total"]-mod_aha["n_solved"]],
                   [std_aha["n_solved"], std_aha["n_total"]-std_aha["n_solved"]]]
    _, p_cross = fisher_exact(table_cross)

    print(f"\n  Fisher exact (mod_aha vs mod_baseline): p={p_mod:.6f}")
    print(f"  Fisher exact (mod_aha vs std_aha): p={p_cross:.6f}")

    all_results["fisher_mod_aha_vs_baseline"] = round(p_mod, 6)
    all_results["fisher_mod_vs_std_aha"] = round(p_cross, 6)

    # Confidence interval (Wilson score)
    from scipy.stats import norm as scipy_norm
    z = scipy_norm.ppf(0.975)  # 95% CI
    ci_results = {}
