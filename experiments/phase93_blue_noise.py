"""
Phase 93: Blue Noise — Temporal Anti-Correlation (ρ < 0)
========================================================

Phase 90 explored positive temporal correlation (red noise, ρ > 0),
finding that all positive-ρ conditions were similar to IID. But
Phase 90 never tested NEGATIVE ρ — anti-correlated "blue noise"
that forces step-to-step direction reversal.

Hypothesis: Anti-correlated noise creates forced DIVERSITY — instead
of exploring one direction persistently, the model is pushed in
opposite directions on consecutive steps, maximizing the exploration
coverage per unit time.

AR(1) formula: z_t = ρ·z_{t-1} + √(1-ρ²)·ε_t

With ρ < 0:
  - Each step's noise tends to oppose the previous step's noise
  - Creates a "zigzag" exploration pattern
  - The net effect is wider coverage of activation space

Conditions (N=30 each):
  1. baseline:     No noise
  2. rho_0.00:     IID noise (ρ=0, standard reference)
  3. rho_-0.50:    Moderate anti-correlation
  4. rho_-0.90:    Strong anti-correlation (extreme zigzag)
  5. rho_0.50:     Positive correlation (Phase 90 replication)
...
class TemporalNoiseHook:
    ...
    For ρ < 0 (blue noise):
      - Each step's noise tends to OPPOSE the previous step
      - Creates "zigzag" exploration: maximizes coverage per unit time
      - Mathematically valid for -1 < ρ < 1
    """
    def __init__(self):
        self.active = False
...
def visualize(all_results):
    ...
    fig.suptitle("Phase 93: Blue Noise — Temporal Anti-Correlation\n"
                 r"AR(1): $z_t = \rho \cdot z_{t-1} + \sqrt{1-\rho^2} \cdot \epsilon_t$, including $\rho < 0$",
    ...
    # Panel 1: Bar chart
    ax = axes[0]
    colors = ["#9E9E9E", "#2196F3", "#00BCD4", "#009688", "#FF9800"]
    ...
    ax.set_title("Solve Rate: Blue vs Red Noise", fontsize=11, fontweight="bold")
    ...
    # Panel 2: ρ vs Solve Rate (full range -1 to +1)
    ax = axes[1]
    ...
    ax.set_xlabel("ρ (Temporal Correlation)", fontsize=12)
    ax.set_ylabel("Solve Rate (%)", fontsize=12)
    ax.set_title("Full ρ Spectrum: Anti-Corr ↔ Correlated", fontsize=11, fontweight="bold")
    ax.set_xlim(-1.0, 0.6)
    ax.legend(fontsize=9)
    ax.grid(alpha=0.3)
    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
    ...
    plt.tight_layout()
    path = os.path.join(FIGURES_DIR, "phase93_blue_noise.png")
...
RHOS = [0.0, -0.50, -0.90, 0.50]

CONDITIONS = [{"name": "baseline", "rho": None}] + \
             [{"name": f"rho_{r:+.2f}".replace("+", "pos").replace("-", "neg"),
               "rho": r} for r in RHOS]


def run_phase93(model, tok, device):
    print(f"\n{'='*80}")
    print(f"  Phase 93: Blue Noise — Temporal Anti-Correlation")
    ...
    pca_candidates = [
        ...
        os.path.join(RESULTS_DIR, "phase90_mistral_pca.npz"),
    ]
    found_pca = False
    ...
    if not found_pca:
        ...
        pca_save = os.path.join(RESULTS_DIR, "phase93_mistral_pca.npz")
    ...
    all_results = {
        "experiment": "Phase 93: Blue Noise — Temporal Anti-Correlation",
        "model": MODEL_SHORT,
        "layer": LAYER_IDX,
        "sigma": BASE_SIGMA,
        "subspace": "PC 257+",
        ...
        "rho_range": "negative (blue noise) and positive (red noise)",
    ...
    for cond_idx, cond in enumerate(CONDITIONS):
        ...
        if rho is None:
            ...
            if abs(rho) > 0:
                autocorr_halflife = -1 / math.log(abs(rho) + 1e-8)
            else:
                autocorr_halflife = 0
            noise_type = "blue" if rho < 0 else ("red" if rho > 0 else "white")
            print(f"    ρ={rho:+.2f} | type: {noise_type} noise | "
                  f"autocorrelation half-life: {autocorr_halflife:.1f} steps")
        ...
        # Intermediate save
        results_path = os.path.join(RESULTS_DIR, "phase93_log.json")
    ...
    if iid:
        ...
        for cond in all_results["conditions"]:
            ...
            if rho is not None and rho < 0:
                ...
                sig = "***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else "n.s."
                print(f"    ρ={rho:+.2f}: {cond['solve_rate']*100:5.1f}% "
...
def main():
    ...
    results, elapsed = run_phase93(model, tok, device)

    print(f"\n{'='*80}")
    print(f"  Phase 93 COMPLETE: {elapsed/60:.1f} min ({elapsed/3600:.1f} hours)")
    print(f"{'='*80}")
