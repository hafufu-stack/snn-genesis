"""
Phase 38: Reverse Transplant — Does Mistral's CfC Degrade Qwen's Math?
========================================================================

Phase 37 showed Qwen CfC → Mistral boosts Math +15%.
This symmetric experiment tests: Mistral CfC → Qwen degrades Math?

If yes → causal proof that "Qwen CfC is Math-competent" (not just noise).
If no  → CfC strategies are universally beneficial regardless of direction.

Experiment Design (mirror of Phase 37):
  1. Train CfC on Mistral (DONOR) → save weights
  2. Free Mistral, load Qwen (HOST)
  3. Train CfC on Qwen natively → evaluate (baseline)
  4. Load Mistral's CfC into Qwen → evaluate (transplant, NO training)
  5. Compare: native vs transplanted + compare with Phase 37

Usage:
    python experiments/phase38_reverse_transplant.py
"""
...
# ═══ Configuration (REVERSED from Phase 37) ═══
DONOR_MODEL = "mistralai/Mistral-7B-Instruct-v0.3"   # Was HOST in Phase 37
DONOR_SHORT = "Mistral-7B"
DONOR_LAYERS = list(range(15, 21))

HOST_MODEL = "Qwen/Qwen2.5-7B-Instruct"               # Was DONOR in Phase 37
HOST_SHORT = "Qwen2.5-7B"
HOST_LAYERS = list(range(14, 20))
...
# ═══ Components (identical to Phase 37) ═══
...
def visualize_bidirectional(p37_data, donor_result, native_result, transplant_result):
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    fig, axes = plt.subplots(2, 2, figsize=(18, 12))
    fig.suptitle("Phase 38: Reverse Transplant — Mistral CfC → Qwen Body\n"
                 "Bidirectional comparison with Phase 37",
                 fontsize=14, fontweight="bold")

    colors = {"factual": "#2ecc71", "creative": "#e74c3c", "math": "#3498db"}

    # Panel 1: σ comparison (Phase 38)
    ax1 = axes[0, 0]
    conditions = [f"Mistral\n(donor)", f"Qwen\n(native)", f"Qwen\n(transplanted)"]
    ...
    ax1.set_title("Phase 38: σ by Task & Condition")
    ax1.legend(fontsize=9)
    ax1.grid(True, alpha=0.3, axis="y")
    ax1.axhline(y=0.074, color="gold", linestyle="--", alpha=0.7)

    # Panel 2: Bidirectional Math Accuracy comparison
    ax2 = axes[0, 1]
    # Phase 37 data
    p37_labels = ["Qwen→Mistral\n(P37 Donor)", "Mistral\n(P37 Native)", "Mistral+QwenCfC\n(P37 Transplant)"]
    p37_math = [p37_data["donor_result"]["math"]["acc"],
                p37_data["native_result"]["math"]["acc"],
                p37_data["transplant_result"]["math"]["acc"]]
    # Phase 38 data
    p38_labels = ["Mistral→Qwen\n(P38 Donor)", "Qwen\n(P38 Native)", "Qwen+MistralCfC\n(P38 Transplant)"]
    p38_math = [donor_result["math"]["acc"], native_result["math"]["acc"], transplant_result["math"]["acc"]]

    x_pos = np.arange(3)
    ax2.bar(x_pos - 0.18, p37_math, 0.35, label="Phase 37 (Qwen→Mistral)", color="#9b59b6", alpha=0.7)
    ax2.bar(x_pos + 0.18, p38_math, 0.35, label="Phase 38 (Mistral→Qwen)", color="#e67e22", alpha=0.7)
    ax2.set_xticks(x_pos)
    ax2.set_xticklabels(["Donor", "Native", "Transplant"])
    ax2.set_ylabel("Math Accuracy (%)")
    ax2.set_title("Bidirectional Math Accuracy")
    ax2.legend(fontsize=9)
    ax2.grid(True, alpha=0.3, axis="y")

    # Panel 3: σ trajectory
    ax3 = axes[1, 0]
    for ttype, color in colors.items():
        nat_s = [t["step"] for t in native_result["sigma_trajectory"] if t["type"] == ttype]
        nat_v = [t["sigma"] for t in native_result["sigma_trajectory"] if t["type"] == ttype]
        ax3.scatter(nat_s, nat_v, c=color, s=15, alpha=0.3, marker="o")
        tra_s = [t["step"] for t in transplant_result["sigma_trajectory"] if t["type"] == ttype]
        tra_v = [t["sigma"] for t in transplant_result["sigma_trajectory"] if t["type"] == ttype]
        ax3.scatter(tra_s, tra_v, c=color, s=30, alpha=0.8, marker="^", label=f"{ttype} (transplant)")
    ax3.axhline(y=0.074, color="gold", linestyle="--", alpha=0.5, linewidth=2)
    ax3.legend(fontsize=8)
    ax3.set_title("Phase 38: σ Trajectory ○=native, △=transplanted")
    ax3.set_xlabel("Step"); ax3.set_ylabel("σ"); ax3.grid(True, alpha=0.3)

    # Panel 4: Grand Verdict
    ax4 = axes[1, 1]; ax4.axis("off")

    p37_delta = p37_data["comparison"]["math_acc_change"]
    p38_delta = transplant_result["math"]["acc"] - native_result["math"]["acc"]

    txt = (f"BIDIRECTIONAL TRANSPLANT ANALYSIS\n"
           f"{'='*42}\n\n"
           f"PHASE 37: Qwen CfC → Mistral\n"
           f"  Math: {p37_data['native_result']['math']['acc']:.0f}% → "
           f"{p37_data['transplant_result']['math']['acc']:.0f}% "
           f"(Δ = {p37_delta:+.0f}%)\n\n"
           f"PHASE 38: Mistral CfC → Qwen\n"
           f"  Math: {native_result['math']['acc']:.0f}% → "
           f"{transplant_result['math']['acc']:.0f}% "
           f"(Δ = {p38_delta:+.0f}%)\n\n"
           f"{'='*42}\n")

    if p37_delta > 0 and p38_delta < 0:
        txt += ("\n🧬 CAUSAL PROOF: ASYMMETRIC TRANSFER!\n"
                "Qwen CfC improves Math → transfers competence\n"
                "Mistral CfC degrades Math → transfers 'giving up'\n"
                "→ CfC carries donor's PERSONALITY, not just noise")
        verdict_color = "#2ecc71"
    elif p37_delta > 0 and p38_delta >= 0:
        txt += ("\n🔬 UNIVERSAL IMPROVEMENT!\n"
                "Both directions improve Math\n"
                "→ CfC strategies are universally beneficial")
        verdict_color = "#3498db"
    else:
        txt += ("\n📊 UNEXPECTED PATTERN\n"
                f"P37: {p37_delta:+.0f}%, P38: {p38_delta:+.0f}%\n"
    ...
    plt.tight_layout()
    fig_path = os.path.join(FIGURES_DIR, "phase38_reverse_transplant.png")
...
def main():
    ...
    # Load Phase 37 results for comparison
    p37_path = os.path.join(RESULTS_DIR, "phase37_brain_transplant_log.json")
    with open(p37_path) as f:
        p37_data = json.load(f)
    print(f"  📂 Phase 37 data loaded (Qwen CfC → Mistral: Math {p37_data['comparison']['math_acc_change']:+.0f}%)")

    # ─── Step 1: Train CfC on DONOR (Mistral) ───
    print("\n" + "🧬"*30)
    print("  STEP 1: Training CfC on DONOR (Mistral)")
    print("🧬"*30)
    ...
    donor_weights_path = os.path.join(WEIGHTS_DIR, "donor_mistral_cfc.pt")
    torch.save({
        "actor_state_dict": donor_actor.state_dict(),
        "critic_state_dict": donor_critic.state_dict(),
    }, donor_weights_path)
    print(f"\n  💾 Donor Mistral CfC weights saved: {donor_weights_path}")

    # Free donor model
    del model, tokenizer
    gc.collect()
    torch.cuda.empty_cache()
    print("  🗑️ Mistral unloaded")

    # ─── Step 2: Load HOST (Qwen) ───
    print("\n" + "🏥"*30)
    print("  STEP 2: Training NATIVE CfC on HOST (Qwen)")
    print("🏥"*30)
    ...
    # ─── Step 3: Transplant Mistral CfC into Qwen ───
    print("\n" + "🧟"*30)
    print("  STEP 3: TRANSPLANTING Mistral CfC into Qwen body!")
    print("🧟"*30)
    ...
    print(f"  🧟 Mistral CfC weights loaded into transplant actor!")

    hook = AdaptiveSNNHook(sigma=0.05)

    transplant_result = run_ppo(model, tokenizer, dataset, transplant_actor, transplant_critic,
                                host_classifier, hook, HOST_LAYERS,
                                f"TRANSPLANT: Mistral CfC → {HOST_SHORT}", do_train=False)

    # ─── Visualization ───
    fig_path = visualize_bidirectional(p37_data, donor_result, native_result, transplant_result)
    ...
    output = {
        "experiment": "Phase 38: Reverse Transplant",
        ...
        "comparison": {
            ...
            "math_acc_change": round(p38_delta, 2),
        },
        "bidirectional": {
            "phase37_direction": "Qwen CfC → Mistral",
            "phase37_math_change": p37_data["comparison"]["math_acc_change"],
            "phase38_direction": "Mistral CfC → Qwen",
            "phase38_math_change": round(p38_delta, 2),
            "causal_asymmetry": p37_data["comparison"]["math_acc_change"] > 0 and p38_delta < 0,
        },
        "figure_path": fig_path,
    }

    log_path = os.path.join(RESULTS_DIR, "phase38_reverse_transplant_log.json")
    ...
    # Grand summary
    print(f"\n{'═'*60}")
    print(f"  🏆 PHASE 38: REVERSE TRANSPLANT — GRAND SUMMARY")
    print(f"{'═'*60}")
    print(f"  DONOR (Mistral CfC on Mistral):")
    print(f"    σ̄={donor_result['mean_sigma_all']:.4f} Math={donor_result['math']['acc']:.1f}%")
    print(f"  NATIVE (Qwen CfC on Qwen):")
    print(f"    σ̄={native_result['mean_sigma_all']:.4f} Math={native_result['math']['acc']:.1f}%")
    print(f"  TRANSPLANT (Mistral CfC on Qwen):")
    print(f"    σ̄={transplant_result['mean_sigma_all']:.4f} Math={transplant_result['math']['acc']:.1f}%")
    print(f"\n  Phase 37: Qwen CfC → Mistral: Math {p37_data['comparison']['math_acc_change']:+.0f}%")
    print(f"  Phase 38: Mistral CfC → Qwen: Math {p38_delta:+.0f}%")
    if p37_data["comparison"]["math_acc_change"] > 0 and p38_delta < 0:
        print(f"\n  🧬 CAUSAL ASYMMETRY CONFIRMED!")
        print(f"     → Qwen CfC carries Math COMPETENCE")
        print(f"     → Mistral CfC carries Math RESIGNATION")
